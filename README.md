...
Install the local job with `python3 scripts/install_launch_agent.py`. The resulting launcher is at `~/Library/LaunchAgents/com.hanhan-mom.weekly-paper-tracker.plist`. It requires a signed-in `gh` and `copilot`, Python 3, and `pdftotext` already on this Mac; it runs when the Mac is awake and your user session is logged in. Every run sends a macOS Notification Center alert for complete, partial, or failed checkpoints without revealing paper titles or URLs. Check Notification Center permissions for `osascript` if alerts do not appear. Errors and details are logged at `~/Library/Logs/weekly-paper-tracker.log`. To inspect the job, run `launchctl print gui/$(id -u)/com.hanhan-mom.weekly-paper-tracker`.
